...
def _row(**overrides):
    base = dict(
        ...
        bbox_long_edge_px=1200.0,  # well above the 512 floor
...
def test_bug_too_small_when_long_edge_below_512px():
    # If the LONG edge is < 512, neither dim has enough pixels to be drawable.
    assert classify_framing(**_row(bbox_long_edge_px=400.0)) == "bug_too_small"
...
def test_multi_bug_takes_priority_over_wide():
    assert classify_framing(**_row(n_distinct_detections=3,
                                    ...
                                    bbox_long_edge_px=600.0)) == "multi_bug"
